...
void OptionsParser::parse()
{
  ...
  if (name.compare("merge") == 0) {
    _matransformation = "merge";
    _merge_nodes = *transform->get_as<bool>("merge_nodes");

    auto io = m_data->get_table("io");
    _meshes = *io->get_array_of<std::string>("inputs");
    _result_file_name = *io->get_as<std::string>("output");

    auto quality = m_data->get_table("quality");
    _compute_quality = *quality->get_as<bool>("compute_quality");
  }
  else if (name.compare("translate") == 0) {
    _matransformation = "translate";
    _coords = *transform->get_array_of<double>("translation");

    auto io = m_data->get_table("io");
    _mesh = *io->get_as<std::string>("input");
    _result_file_name = *io->get_as<std::string>("output");
